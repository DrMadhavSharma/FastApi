            ...
            <ul>
              {search.users.map(u => (
                <li key={u.id} style={{ marginBottom: 6 }}>
                  {u.username} ({u.role}) â€” {u.email} {u.is_active ? "" : "[inactive]"}
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul>
              {search.doctors.map(d => (
                <li key={d.id} className="row-doctor" style={{ marginBottom: 6 }}>
                  <div className="row">
                     <span>#{d.id} • user:{d.user_id} • {d.specialization}</span>
                      <div className="row-actions">
                         <button className="btn" onClick={() => openModal("doctor","edit",d)}>edit</button>
                         <button className="btn" onClick={() => blacklistUser("doctor", d.id)}>blacklist</button>
                         <button className="btn btn-danger" onClick={() => deleteEntity("doctor", d.id)}>delete</button>
                      </div>
                   </div>
               </li>
              ))}
