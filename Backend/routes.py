...
def admin_add_doctor(doctor: DoctorCR, user=Depends(get_current_user), db: Session = Depends(get_session)):
    require_admin(user)
    existing_user = db.queryute(select(User).where(User.email == doctor.email)).scalar_one_or_none()
    if existing_user:
...
def admin_add_patient(patient: PatientCR, user=Depends(get_current_user), db: Session = Depends(get_session)):
    require_admin(user)
    existing_user = db.queryute(select(User).where(User.email == patient.email)).scalar_one_or_none()
    if existing_user:
        raise HTTPException(status_code=400, detail="Email already registered")
    db_user = User(
...
def admin_search(q: str, user=Depends(get_current_user), db: Session = Depends(get_session)):
    require_admin(user)
    users = db.queryute(select(User).where(User.username.ilike(f"%{q}%"))).scalars().all()
    doctors = db.queryute(select(Doctor).join(User).where(User.username.ilike(f"%{q}%"))).scalars().all()
    patients = db.queryute(select(Patient).join(User).where(User.username.ilike(f"%{q}%"))).scalars().all()
    return {
...
def get_doctor_user(db: Session, user_email: str):
    usr = db.queryute(select(User).where(User.email == user_email)).scalar_one_or_none()
    if not usr or usr.role != RoleEnum.doctor:
        raise HTTPException(status_code=403, detail="Doctor only")
    doc = db.queryute(select(Doctor).where(Doctor.user_id == usr.id)).scalar_one_or_none()
    if not doc:
        raise HTTPException(status_code=404, detail="Doctor profile not found")
    return usr, doc
...
def doctor_appointments(user=Depends(get_current_user), db: Session = Depends(get_session)):
    ...
    appts = db.queryute(select(Appointment).where(Appointment.doctor_id == doc.id)).scalars().all()
    result = []
    for a in appts:
        result.append({
            "id": a.id,
...
def get_patient_history(patient_id: int, user=Depends(get_current_user), db: Session = Depends(get_session)):
    usr, doc = get_doctor_user(db, user["username"]) 
    import json
    appts = db.queryute(select(Appointment).where(Appointment.patient_id == patient_id, Appointment.doctor_id == doc.id)).scalars().all()
    history = []
    for a in appts:
...
def add_patient_history(patient_id: int, payload: HistoryEntry, user=Depends(get_current_user), db: Session = Depends(get_session)):
    usr, doc = get_doctor_user(db, user["username"]) 
    appt = db.queryute(select(Appointment).where(Appointment.patient_id == patient_id, Appointment.doctor_id == doc.id).order_by(Appointment.appointment_date.desc())).scalars().first()
    if not appt:
...
def get_patient_user(db: Session, email: str):
    usr = db.queryute(select(User).where(User.email == email)).scalar_one_or_none()
    if not usr or usr.role != RoleEnum.patient:
        raise HTTPException(status_code=403, detail="Patient only")
    pat = db.queryute(select(Patient).where(Patient.user_id == usr.id)).scalar_one_or_none()
    if not pat:
...
def patient_specializations(user=Depends(get_current_user), db: Session = Depends(get_session)):
    ...
    specs = db.queryute(select(Doctor.specialization).distinct()).scalars().all()
    return specs
...
def patient_doctors(user=Depends(get_current_user), db: Session = Depends(get_session)):
    usr, pat = get_patient_user(db, user["username"]) 
    import json
    rows = db.queryute(select(Doctor, User).join(User, User.id == Doctor.user_id)).all()
    out = []
    for d, u in rows:
        avail = []
        if d.availability:
...
def patient_appointments(user=Depends(get_current_user), db: Session = Depends(get_session)):
    usr, pat = get_patient_user(db, user["username"]) 
    import json
    appts = db.queryute(select(Appointment).where(Appointment.patient_id == pat.id)).scalars().all()
    out = []
    for a in appts:
